...
- `LESSONS_LEARNED.md`: Erkenntnisse und Best Practices zur 3D-Modell-UV-Aufbereitung.
...
- Der initiale Flow ist Prompt-first: Nutzer geben zuerst den Prompt ein und erzeugen daraus eine KI-Kachel. Unter dem Prompt-Feld stehen klickbare Beispielideen (`promptIdeas` in `src/App.tsx`), die das Feld mit einer ausformulierten Musterbeschreibung fuellen. Der Erzeugen-Button ist unter `minPromptLength` (8 Zeichen) deaktiviert; bei 1-7 Zeichen erklaert ein Hinweistext, warum.
...
- In der Versionsliste können die exakten englischen Prompts über ein ausklappbares Akkordeon-Panel (Info-Icon) direkt im UI eingesehen und per Button kopiert werden (Clipboard-API mit `execCommand`-Fallback in `copyTextToClipboard`). Im selben Akkordeon steht der Block `Farbwelt` mit den aus der jeweiligen Variante analysierten Hauptfarben (`version.settings.colors`, Swatch + Hex, Klick kopiert). Bewusst hier und nicht im Anpassungen-Panel: Letzteres enthält ausschliesslich veraendernde Controls, die Farbwelt ist read-only-Info und gehoert zu den Fakten der Variante (so bleibt sie zudem pro Variante vergleichbar).
- Varianten zeigen ihre Zusatzbeschreibung (`Version.note`, der Prompt-Chat-Wunsch) als Untertitel in der Liste, damit sie unterscheidbar bleiben. Fertige Varianten sind über ein Hover-/Fokus-Papierkorb-Icon löschbar; die letzte fertige Variante ist geschützt (kein Lösch-Icon), und beim Löschen der aktiven Variante wird die nächste fertige übernommen. Die Nummerierung neuer Varianten nutzt `nextVersionName()` (höchste vorhandene Nummer + 1), nicht die Listenlänge – sonst entstünden nach dem Löschen doppelte Namen.
...
- Das aktuelle Kleid-Asset ist `public/models/custom-summer-dress-new-uv.glb`, erzeugt aus `incoming/kleid-neueversion.glb` und in Blender mit Material `fabric` sowie einer `Fabric_UV`/`UVMap` aufbereitet. Der Pfad ist in `garmentTypes.kleid.modelPath` in `src/App.tsx` hinterlegt. **Polygon-reduziert** fuer Performance (siehe unten): von ~321.000 auf ~80.000 Dreiecke (Datei 34,6 MB -> 1,8 MB), UVs/Form per Checker-Render verifiziert unveraendert.
- Das aktuelle Midikleid-Asset ist `public/models/midi-dress.glb`, erzeugt aus `incoming/marvelous_designer_11__midi_dress__clo3d.glb` (Sketchfab/Marvelous-Designer-Export mit fertigen UVs). Das Quell-GLB enthielt fuenf Materialien mit irrefuehrenden Namen: `Material78181` ist das eigentliche Kleid, `FABRIC_1_FRONT_78193` nur eine duenne Saumkante, `W_Shoes_skinSG1SG1SG1`/`HEELSG1SG1`/`dummySG1SG1` sind Schuhe/Absaetze. Welches Material das Kleid ist, wurde per Material-Isolations-Renders bestimmt (nicht ueber den Namen). Das Aufbereitungs-Script behaelt nur das Kleid-Mesh, loescht Schuhe/Absaetze/Saumkante (das Modell hat keinen Koerper/Beine, schwebende Schuhe wirken defekt) und benennt das Material zu `fabric` (doppelseitig) um. Wichtig: Die mitgelieferten Marvelous-Designer-UVs sind **Schnittteil-Inseln mit ungleicher Texeldichte** (Oberteil ~1,8x dichter als die Rockbahnen), wodurch die gekachelte Textur als getrennte Flaechen mit unterschiedlichem Rapport erscheint. Deshalb werden die MD-UVs verworfen und per **Box-/Wuerfelprojektion** (`uv.cube_project`) durch eine einheitliche, welt-aufrechte `UVMap` ersetzt.

  **Lehren und Best Practices aus der UV-Korrektur des Midikleids:**
  1. *Stauchungsvermeidung (Aspektverhältnis):* `bpy.ops.uv.cube_project` darf **nicht** mit `scale_to_bounds=True` ausgeführt werden. Da das Kleid höher als breit ist, skaliert Blender U und V unabhängig voneinander, was zu einer massiven horizontalen Stauchung der Textur führt. Die Projektion muss mit `scale_to_bounds=False` erfolgen, gefolgt von einer manuellen Normalisierung in Python: Beide Achsen U/V werden durch den maximalen Ausdehnungsfaktor `max_dim = max(width, height)` geteilt, um das 1:1-Verhältnis zu wahren.
  2. *Naht-Risse (Mipmapping-Bleeding):* UV-Koordinaten dürfen nicht negativ oder außerhalb von `[0, 1]` liegen. Solche Werte führen auf Grafikprozessoren an den UV-Nahtkanten bei Mipmapping zu Rundungsfehlern, die sich als unschöne dünne weiße Risslinien/Streifen zeigen. Die UVs müssen durch Subtraktion des Minimums (`min_u`, `min_v`) und zentrierte Aufbereitung komplett in den positiven Bereich `[0, 1]` verschoben werden.
  3. *Ausrichtung (V-Flip):* Standardmäßige Blender-Koordinaten müssen für WebGL/Three.js gespiegelt werden, da der standardmäßige glTF-Exporteur die V-Achse invertiert. Die korrekte mathematische Formel im Python-Skript lautet `v_final = 1.0 - v_norm` (ausgehend von den normierten Werten). Ein einfaches Vorzeichen-Flippen (`uv.y = -uv.y`) erzeugt ungültige negative Werte und führt zu Darstellungsfehlern.
  4. *Blender-Python API-Staleness:* Nach jedem Wechsel der Modi (Edit -> Object Mode) oder nach topologischen Operationen (z. B. `remove_doubles` Weld) werden interne Blender-C++-Zeiger neu zugewiesen. Zuvor gespeicherte Python-Referenzen auf UV-Layer-Arrays werden dadurch **stale** und geben willkürliche Garbage-Fließkommawerte (z. B. `10^37`) zurück. Referenzen müssen nach Moduswechseln zwingend neu geholt werden: `uv_layer = dress_obj.data.uv_layers.active`.

  Der Pfad ist in `garmentTypes.midikleid.modelPath` in `src/App.tsx` hinterlegt. Zwei Lehren fuer neue Sketchfab/MD-Assets: (1) Materialnamen nicht vertrauen, das Stoff-Mesh per Isolations-Render verifizieren; (2) MD-Schnittteil-UVs nicht direkt fuer Rapport-Vorschau nutzen, sondern einheitlich neu projizieren.

- Das aktuelle Hosen-Asset ist `public/models/custom-trousers-uv.glb`, erzeugt aus `incoming/anime-black-trousers.zip` (Meshy-GLB nur mit `POSITION`, ohne UVs/Material/Normalen). In Blender (headless) wurden Normalen neu berechnet, Material `fabric` (doppelseitig) zugewiesen und per **Box-/Wuerfelprojektion** (`uv.cube_project`, `cube_size` = Modellhoehe, `correct_aspect=True`) eine `UVMap` erzeugt. Die Box-Projektion richtet das Muster auf den vertikalen Bein-/Hueftflaechen welt-aufrecht aus (V = Hoehenachse) und legt die Projektionsnaht an die seitlichen Achsenkanten, nicht auf Vorder-/Rueckseite. Wichtig: `cube_project` liefert eine invertierte V-Achse, wodurch das Muster im Viewer auf dem Kopf stuende – das Aufbereitungs-Script spiegelt deshalb alle UV-V-Koordinaten (`uv.y = -uv.y`), damit Motive aufrecht stehen. Der Pfad ist in `garmentTypes.hose.modelPath` in `src/App.tsx` hinterlegt. Das alte, untexturierbare `anime-black-trousers.glb` wird nicht mehr referenziert. **Polygon-reduziert** fuer Performance (siehe unten): von ~479.000 auf ~72.000 Dreiecke (Datei 14,9 MB -> 1,95 MB). Beide Materialien (`fabric`, `button`) bleiben dabei erhalten, sodass der Knopf-Filter im Viewer weiter greift.

- **Polygon-Reduktion grosser GLB-Assets (Performance auf aelteren Rechnern):** Kleid und Hose kamen extrem hoch tesselliert und komplett **unwelded** an (jedes Dreieck eigene Vertices, z. B. Kleid 962k Vertices bei 321k Dreiecken). Dadurch ist jede Kante eine Border-Kante, und weder `gltf-transform simplify` noch `weld` reduzieren etwas (gltf-transform v4 `weld` ist bitweise und kennt keine Toleranz mehr). Die robuste, hier verwendete Pipeline laeuft headless ueber Blender: (1) `bmesh.ops.remove_doubles(dist=0.0001)` verschweisst die Topologie tolerant (UVs liegen per-loop vor und bleiben als Inseln/Seams erhalten); (2) `DECIMATE`-Modifier im `COLLAPSE`-Modus mit Ziel-`ratio` und `delimit={'NORMAL','UV','SEAM','SHARP','MATERIAL'}`, damit Silhouette, Rapport und Materialgrenzen (z. B. Hosenknopf) geschuetzt bleiben; (3) `shade_smooth` und GLB-Export. Vor dem Uebernehmen mit einem Checker-Material aus Front/3-4/Seite/Rueckseite rendern und gegen das Original vergleichen (UV-Verzerrung, Silhouette), zusaetzlich Materialnamen per `inspect` pruefen. Midikleid (12k) und Hoodie (30k) sind bereits schlank und bleiben unveraendert. Kein Draco-Compression noetig; die Reduktion kommt aus Topologie + Decimate.
...
- Die drei Ansichts-Regler sind rein CSS- bzw. SVG-basiert:
  ...
  - `Horizontaler Versatz` (`offsetX`, 0-100) und `Vertikaler Versatz` (`offsetY`, 0-100): aendern `background-position-x/y` in `makeFabricStyle`, um das Muster in der Stoffbahn zu verschieben. Reiner Vorschau-Effekt, kein KI-Signal. Nur in der Stoffbahn-Ansicht aktiv. Anzeige mit Einheit `%`.
- Weitere reine Ansichts-Controls (kein KI-Signal, in `sessionStorage` gespiegelt):
  - `Kachelgrenzen anzeigen` (`showTileBorders`, nur Nahtpruefung, Default an): blendet die 1px-Hilfslinien auf den Stosskanten der 3x3-Wiederholung aus (`.tile-repeat-grid.borders-hidden`). Ausblendbar, weil die Linien sonst genau die Naht verdecken, die geprueft werden soll.
  - `Zuruecksetzen` im Block Bildeinstellungen: erscheint nur, wenn Helligkeit/Kontrast/Saettigung vom Neutralwert abweichen, und setzt alle drei zurueck (auch in der aktiven Variante).
...
Die **Stoffart-Umschaltung** (`materialPreset`: Standard/Seide/Leinen/Sport) im Panel "3D-Praesentation" der Kleidungsansicht ist aus der UI **ausgeblendet**, bis die Materialdarstellung ueberzeugt; aktuell bietet "3D-Praesentation" nur noch die `Lichtstimmung` (`lightingPreset`). State, Typ (`PatternSettings`-naher Top-Level-State), Session-Spiegelung und die Uebergabe an `GarmentPreview3D` bleiben erhalten; der `useState` wird ohne Setter destrukturiert. Zum Reaktivieren den Setter ergaenzen und den Stoffart-Block wieder einsetzen. Nicht ohne ueberzeugende Materialwirkung sichtbar machen.
...
- Die analysierte Palette wird im ausklappbaren Detail-Panel der jeweiligen Variante (Versionsliste, Info-Icon) als Block `Farbwelt` angezeigt (Swatch + Hex-Wert, zur Orientierung, nicht farbverbindlich). Ein Klick kopiert den Hex-Wert in die Zwischenablage mit kurzem "Kopiert!"-Feedback. Nicht ins Anpassungen-Panel zuruecklegen: das buendelt nur veraendernde Controls, die Farbwelt ist read-only-Info je Variante.
...
- Der `preview-header` ist `flex-wrap: wrap`: bei schmaler Stage (z. B. 1280px-Laptop) brechen die Werkzeuge unter den Titel um, statt rechts unter das Seitenpanel zu laufen. Nicht auf `nowrap` zuruecksetzen.
- Im Desktop-Grid (`min-width: 1181px`) scrollt das Anpassungen-Panel bei Fensterhoehen bis 980px intern (`overflow-y: auto`), damit z. B. die Lichtstimmung erreichbar bleibt. In diesem Modus werden die Tooltip-Popups des Panels an der vollen Zeilenbreite ausgerichtet (statt am Icon) und der Pfeil ausgeblendet, weil der Scroll-Container sonst ueberstehende Popups abschneiden wuerde.
